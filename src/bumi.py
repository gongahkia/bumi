    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if response and response.status == 200:
                ...
                result["error"] = f"Unexpected status: {response.status if response else 'no response'}"
...
def export_to_csv(data, output_file, data_key=None):
    ...
    flattened = [flatten_dict(item) if isinstance(item, dict) else {"value": item} for item in items]
...
class PostgreSQLStorage:
    ...
    def __init__(self, host="localhost", port=5432, database="bumi", user="postgres", password=""):
        """
        Args:
            host: database host
            port: database port
...
def create_api_server():
    ...
    async def scrape_batch_endpoint(request: BatchScrapeRequest, background_tasks: BackgroundTasks):
    ...
    async def scrape_watchlist_endpoint(username: str, paginate: bool = True, max_pages: int = 50):
        ...
        try:
            ...
            target_url = f"https://letterboxd.com/{username}/"
            result = scrape_letterboxd_user_watchlist(target_url, paginate=paginate, max_pages=max_pages)
            return JSONResponse(content={"username": username, "watchlist": result})
        except BumiException as e:
            raise HTTPException(status_code=500, detail=str(e))

    @app.get("/scrape/diary/{username}")
    async def scrape_diary_endpoint(username: str, paginate: bool = True, max_pages: int = 50):
        ...
        try:
            ...
            target_url = f"https://letterboxd.com/{username}/"
            result = scrape_user_diary(target_url, paginate=paginate, max_pages=max_pages)
            return JSONResponse(content={"username": username, "diary": result})
        except BumiException as e:
            raise HTTPException(status_code=500, detail=str(e))

    @app.get("/scrape/reviews/{username}")
    async def scrape_reviews_endpoint(username: str, paginate: bool = True, max_pages: int = 50):
        ...
        try:
            ...
            target_url = f"https://letterboxd.com/{username}/"
            result = scrape_user_reviews(target_url, paginate=paginate, max_pages=max_pages)
        ...
        for webhook_id, webhook in targets:
            if async_send:
                thread = threading.Thread(
                    ...
                    args=(webhook_id, webhook, payload)
                )
                thread.daemon = True
                thread.start()
            else:
    ...
    def _send_notification(self, webhook_id, webhook, payload):
        ...
        try:
            ...
            with urlopen(request, timeout=30) as response:
                print(f"Webhook {webhook_id}: sent to {webhook['url']}, status {response.status}")
...
def scrape_with_webhook(target_url, webhook_url=None, paginate=True):
    ...
    try:
        result = scrape_letterboxd(target_url, paginate=paginate)
        notify_webhook("scrape_complete", {
            "job_id": job_id,
            "target_url": target_url,
            "success": True,
            "result": result,
